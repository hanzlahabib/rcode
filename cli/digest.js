 ...
 *   1. rihal/digests/<name>.md            (curated 20-line digest)
 *   2. rihal/agents/rihal-<name>.md       (fallback: full agent file)
 *
 * Accepts both `sadiq` and `rihal-sadiq` as input (rihal- prefix stripped).
 */
...
function listAvailable(digestDir, agentsDir) {
  ...
  const agentNames = fs.existsSync(agentsDir)
    ? fs.readdirSync(agentsDir)
        .filter((f) => f.endsWith('.md') && f.startsWith('rihal-'))
...
function resolveAgentFile(name, digestDir, agentsDir) {
  ...
  const agentFile = path.join(agentsDir, `rihal-${name}.md`);
