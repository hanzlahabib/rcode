 ...
 *
 * `agents` are the bare names (no rcode- prefix, no .md) of the agent files
 *   under rcode/agents/*.md — these are what install ships to
 *   .claude/agents/rcode-<name>.md, .cursor/rules/rcode/agents/, etc.
 *   NOT the phrase-activated agent skills under rcode/skills/agents/.
 * `actions` are the skill dir names under rcode/skills/actions/, plus the
...
function readPackageManifest(packageRoot) {
  ...
  // Issue #783 — agents are file-based (rcode/agents/*.md), not the skill
  // directories under rcode/skills/agents/. The installer ships rcode/agents/
  ...
  if (fs.existsSync(agentsDir)) {
    for (const entry of fs.readdirSync(agentsDir, { withFileTypes: true })) {
      ...
      if (!entry.name.startsWith('rcode-') || !entry.name.endsWith('.md')) continue;
      manifest.agents.add(entry.name.replace(/^rcode-/, '').replace(/\.md$/, ''));
    }
  }
...
/**
 * Verify a Claude install. Agents live at .claude/agents/rcode-<name>.md.
 * Action skills live at .claude/skills/<name>/ (bare name, no rcode- prefix).
 *
 * Note: .claude/skills/ ALSO contains rcode-<name>/ directories that are
...
function verifyClaudeInstall(cwd, packageRoot, options = {}) {
  ...
  // Agents: .claude/agents/rcode-<name>.md (file-based, not dir-based).
  const installedAgents = new Set();
  if (fs.existsSync(agentsDir)) {
    for (const f of fs.readdirSync(agentsDir)) {
      if (f.startsWith('rcode-') && f.endsWith('.md')) {
        installedAgents.add(f.replace(/^rcode-/, '').replace(/\.md$/, ''));
      }
    }
  }
  ...
  // level .claude/agents/rcode-*.md when the user's ~/.claude/ already has
  ...
  // Actions: .claude/skills/rcode-<name>/. installSkills (cli/install.js)
  // prefixes every action with rcode-, and readPackageManifest does the
  // same — so both sides are normalized. The previous version filtered OUT
  // rcode-* dirs which excluded ALL real actions and made the diff always
 ...
 * .mdc agent rule per rcode/agents/*.md into the NESTED directory
 *   .cursor/rules/rcode/agents/rcode-<name>.mdc   (cursor)
 *   .windsurf/rules/rcode/agents/rcode-<name>.mdc (windsurf)
...
function verifyRulesInstall(editor, cwd, packageRoot) {
  ...
  if (fs.existsSync(agentsDir)) {
    for (const entry of fs.readdirSync(agentsDir, { withFileTypes: true })) {
      ...
      if (!entry.name.startsWith('rcode-') || !entry.name.endsWith('.mdc')) continue;
      installed.add(entry.name.replace(/^rcode-/, '').replace(/\.mdc$/, ''));
    }
  }
 ...
 * to the NESTED .antigravity/rcode/agents/rcode-<name>.md — not
...
function verifyAntigravityInstall(cwd, packageRoot) {
  ...
  if (fs.existsSync(agentsDir)) {
    for (const entry of fs.readdirSync(agentsDir, { withFileTypes: true })) {
      if (!entry.isFile()) continue;
      if (!entry.name.startsWith('rcode-') || !entry.name.endsWith('.md')) continue;
      installed.add(entry.name.replace(/^rcode-/, '').replace(/\.md$/, ''));
    }
  }
