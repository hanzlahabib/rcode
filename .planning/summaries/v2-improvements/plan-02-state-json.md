---
plan: "02"
title: state.json — project memory and /rcode-status dashboard
priority: critical
depends_on: ["01"]
...
Create `.rcode/state.json` as the canonical project memory store and a `/rcode-status` slash command that prints a readable dashboard from it. Right now every council session starts blind — no memory of decisions, phases, or what's been executed.

## Context

- `rcode-tools.cjs` already has `state advance-plan` and `state record-execution` stubs from Plan 01
- State is written to `.rcode/state.json` in the installed project (not in the rcode repo itself)
- The council workflow already saves session artifacts to `.planning/council-sessions/` — status should surface those too
- Template for the initial state file needs to live at `rcode/v2/` so `install-v2.js` can seed it on fresh installs
...
- `last_session` — ISO timestamp, updated on every rcode workflow run
...
type: auto
**Steps:**
1. Create `rcode/v2/state.json` with the schema above, all values at their defaults:
   ```json
   {
     "version": "1",
...
2. Add copy step in `install-v2.js`: copy `rcode/v2/state.json` → `.rcode/state.json`, replacing `__PROJECT_NAME__` with the project name from config and `__INSTALL_DATE__` with current ISO timestamp.
3. Skip the copy if `.rcode/state.json` already exists (don't overwrite on re-install).
**Done when:** fresh `install-v2` creates a valid `.rcode/state.json` with correct project name
**Commit:** `feat(state): add state.json template and seed on install`

### Task 2 — Implement full state subcommands in rcode-tools.cjs
type: auto
**Steps:**
1. Read the current `rcode-tools.cjs` to see what state stubs exist from Plan 01
2. Implement all missing state subcommands:

   **`state read`** — print full state.json as formatted JSON to stdout

   **`state init`** — create state.json at `.rcode/state.json` if missing. Accept `--project <name>` flag.
...
**Done when:** all subcommands work against a real `.rcode/state.json`
**Commit:** `feat(cli): implement full state subcommands in rcode-tools`

### Task 3 — Wire council workflow to auto-update state after session save
type: auto
**Steps:**
1. Read `rcode/v2/workflows/council.md` — find Step 5 (session save)
...
4. Add a note in the workflow: "If rcode-tools.cjs state commands fail (state.json missing), continue without error — state is optional, session artifact is mandatory."
...
### Task 4 — Create /rcode-status slash command and workflow
type: auto
**Steps:**
1. Create `rcode/v2/commands/status.md` — the slash command definition:
   ```yaml
   ---
   name: rcode-status
   ...
   ---
   ```
2. Create `rcode/v2/workflows/status.md` — the workflow:

   **Step 1:** Run `node .rcode/bin/rcode-tools.cjs state read` → parse JSON
...
3. Add `status.md` to `install-v2.js` copy steps (→ `.claude/commands/rcode/status.md` and `.rcode/workflows/status.md`)
4. Register `/rcode-status` in skills manifest (same pattern as council and execute)
**Done when:** `/rcode-status` prints the dashboard from a real state.json
**Commit:** `feat(workflows): add /rcode-status dashboard command`

### Task 5 — Wire execute workflow to update state after plan completion
type: auto
**Steps:**
1. Read `rcode/v2/workflows/execute.md`
...
## Success criteria
- [ ] Fresh install creates `.rcode/state.json` with correct project name and ISO date
- [ ] Re-install does not overwrite existing `state.json`
- [ ] `rcode-tools.cjs state read` returns valid JSON
- [ ] `rcode-tools.cjs state record-council` appends to `council_sessions[]`
- [ ] Council workflow auto-updates state after session save
- [ ] Execute workflow auto-updates state after plan completion
- [ ] `/rcode-status` prints readable dashboard from state.json
- [ ] `/rcode-status` on missing state.json prints a clean "no state" message, not an error
