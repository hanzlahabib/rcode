...
rcode v2 installs one monolithic `core` module. As the agent roster grows (executor, planner, new specialists), users shouldn't have to install everything. This plan introduces a lightweight module system where capabilities are grouped into installable bundles, and `/rcode-install <module>` adds them to an existing project.
...
- A module is a named bundle of: agents, workflows, commands, references, rcode-tools extensions
...
|---|---|---|
| `core` | council agents (5), /rcode-council, /rcode-discuss, /rcode-status | Always installed |
| `execution` | rcode-executor, rcode-planner, /rcode-execute, /rcode-plan | Opt-in |
...
Each module is defined in `rcode/v2/modules/{name}.yaml`:

```yaml
# rcode/v2/modules/execution.yaml
name: execution
version: "1.0"
description: "Plan execution — write and run PLAN.md files with rcode-executor and rcode-planner"
...
type: auto
**Steps:**
1. Create `rcode/v2/modules/core.yaml` — lists everything currently installed by install-v2.js under core:
...
2. Create `rcode/v2/modules/execution.yaml` — lists Plan 01 + Plan 03 outputs:
   - Agents: rcode-executor.md, rcode-planner.md
...
2. Refactor the copy logic:
   - Instead of hardcoded copy lists, read `rcode/v2/modules/core.yaml`
...
### Task 3 — Add /rcode-install slash command
type: auto
**Steps:**
1. Create `rcode/v2/commands/install.md`:
   ```yaml
   ---
   name: rcode-install
   description: Install a rcode module into the current project
   argument-hint: "<module-name>"
   allowed-tools: [Read, Bash]
   ---
   ```
2. Create `rcode/v2/workflows/install.md`:
   ...
   **Step 2:** Check `requires` — are required modules installed? If not: print "Module {name} requires {dep} to be installed first. Run /rcode-install {dep}." and exit.
   ...
   **Step 5:** Print summary:
   ...
   /rcode-execute — {description}
   /rcode-plan — {description}
   ```

3. Note: this workflow requires the rcode package to be accessible. Document the requirement.
4. Add to install-v2.js: copy `install.md` command and workflow as part of core install (so users can always run /rcode-install after initial setup)
**Done when:** workflow and command files exist
**Commit:** `feat(workflows): add /rcode-install module management command`

### Task 4 — Extend rcode-tools.cjs with module subcommands
type: auto
**Steps:**
1. Add `module list` — reads `rcode/v2/modules/*.yaml` from the package, prints available modules with names, descriptions, and installed status (cross-referenced with `.rcode/_config/manifest.yaml`)
...
**Commit:** `feat(cli): add module list/installed/check-requires to rcode-tools`

### Task 5 — Integration test: install execution module on a fresh project
type: checkpoint:human-verify
**What executor does:** Runs fresh install (core only), then `/rcode-install execution`, then checks installed files.
**Verify:**
- Before: `.claude/agents/rcode-executor.md` does not exist
- After `/rcode-install execution`: `.claude/agents/rcode-executor.md` exists
- After: `.claude/commands/rcode/execute.md` exists
- After: `manifest.yaml` contains `execution` in modules list
- After: `/rcode-execute` is accessible as a slash command
- Core commands (`/rcode-council`, `/rcode-status`) still work after module install
...
- [ ] `/rcode-install execution` works end-to-end in an installed project
