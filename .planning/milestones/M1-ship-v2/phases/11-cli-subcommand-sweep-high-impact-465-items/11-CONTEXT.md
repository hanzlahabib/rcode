...
4. `context refresh` — refresh `.rcode/context/` cache (used by init.md)
...
- **D-7:** `context refresh` clears + rebuilds the in-project context cache from `.rcode/sources.yaml`. No-op gracefully if no sources configured.
...
- `rcode/workflows/new-project-roadmap.md` — consumes generate-claude-md
- `rcode/workflows/check-implementation-readiness.md` — consumes self-named subcommand
- `rcode/workflows/execute-sprint.md` — consumes commit-to-subrepo
- `rcode/workflows/init.md` — consumes context refresh
- `rcode/workflows/ui-phase.md` — consumes classify-tech
- `rcode/bin/rcode-tools.cjs` — host file for all new cmd functions
