...
phase: 9
plan_number: 1
title: dogfood scan — invoke each rcode tool on rcode, document gaps
wave: 1
depends_on: []
files_modified:
...
<objective>
Run every user-facing rcode tool (`/rcode-health`, `/rcode-status`, `/rcode-audit`, `/rcode-feature-drift`, `/rcode-memory-audit`, `/rcode-scan`, `/rcode-progress`, `/rcode-stats`, `state sync --from-disk`) against rcode itself. Capture each tool's output, compare to its declared purpose, surface drift. File a GH issue for every gap found.
</objective>

<must_haves>
- Single artifact: `.planning/phases/9-dogfood-audit-pass/SCAN-RESULTS.md`
- One section per tool: `## /rcode-<name>`, with subsections "Claimed purpose", "Observed output (truncated)", "Drift / gaps", "Severity" (breaking | shape | cosmetic | none)
...
<read_first>
- rcode/workflows/health.md
- rcode/workflows/status.md
- rcode/workflows/scan.md (if present)
- rcode/workflows/progress.md
- rcode/workflows/stats.md (if present)
</read_first>

<action>
For each tool, run the CLI command that backs it (typically `node rcode/bin/rcode-tools.cjs <subcommand>`). Capture stdout to a temp variable, then write to SCAN-RESULTS.md under that tool's heading.

Concrete commands to run:
- `node rcode/bin/rcode-tools.cjs init phase-op 0` — for status / phase-state introspection
- `node rcode/bin/rcode-tools.cjs state sync --from-disk` — should return warnings:[] post-#455
- `node rcode/bin/rcode-tools.cjs roadmap list-phases`
- `node rcode/bin/rcode-tools.cjs plan list`
- `node rcode/bin/rcode-tools.cjs help` — confirm `phase add` line is present (post-#460)
...
<task id="9.1.2">
<title>Run /rcode-feature-drift and /rcode-memory-audit on rcode itself (read-only)</title>
<read_first>
- rcode/workflows/feature-drift.md (just shipped)
- rcode/workflows/memory-audit.md (Phase 6 extended)
</read_first>
...
1. **For feature-drift:** check that the workflow file is parseable, that `rcode-docs-auditor` has the `<mode_feature_drift>` section, and that the slash-command file exists. Don't actually spawn the agent (that's Phase 10+ work or scheduled cadence).
2. **For memory-audit:** check the `--fix` flag detection block exists in the workflow. Run a dry-run by reading the workflow's "Steps" section and confirming each referenced file exists in `.rcode/memory/`.

Document findings in SCAN-RESULTS.md under the same 4-subheading structure. If `.rcode/memory/INDEX.md` doesn't exist, note that the memory-audit precondition would fail — this IS a finding (severity: shape) since the workflow assumes it.
</action>

<acceptance_criteria>
- SCAN-RESULTS.md gains two new sections: `## /rcode-feature-drift` and `## /rcode-memory-audit`
