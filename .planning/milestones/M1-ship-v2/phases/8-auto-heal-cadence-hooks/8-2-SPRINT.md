...
files_modified:
  ...
  - rcode/skills/core/rcode-enable-hooks/SKILL.md
...
<objective>
Add `--quick` flag to feature-drift workflow + ship a PostToolUse hook installable via `/rcode-enable-hooks` that fires drift detection on doc edits.
</objective>
...
- `rcode/skills/core/rcode-enable-hooks/SKILL.md` (or equivalent) gains a section advertising the new hook (extend, don't replace)
</must_haves>
...
<action>
In `rcode/workflows/feature-drift.md` parse_args step, add detection for `--quick`. In severity_classify step, document the safety rule: `--quick` mode forces report-only regardless of other flags. Add to guardrails: "QUICK_MODE always implies report-only — never patches even if --fix is also passed."
</action>
...
<action>
Write a doc explaining the PostToolUse hook config + opt-in via `/rcode-enable-hooks`. Include the literal `.claude/settings.json` JSON block users can paste, the file-pattern matchers (docs/, prd/, epics/, stories/, .planning/), and the safety promise that `--quick` never modifies files.
</action>
