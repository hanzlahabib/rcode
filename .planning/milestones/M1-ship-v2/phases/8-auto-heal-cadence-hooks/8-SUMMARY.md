...
| 8.2 | `rcode/workflows/feature-drift.md` (extended), `docs/HOOKS-AUTO-HEAL.md` (new) | `--quick` flag added; hard-coded to suppress `--fix`. Hooks doc shows the `.claude/settings.json` JSON block + opt-in path. PostToolUse only — never blocks edits. |
| 8.3 | `rcode/workflows/feature-drift.md` (`--mode=phase-status` branch), `rcode/agents/rcode-docs-auditor.md` (`<mode_phase_status>`), `scripts/dogfood-check.sh` (Check 6) | Closes **#461** — third drift dimension live. Severity rules (trivial / partial / major) enforced; auto-fix only patches trivial (✅ marker, missing date), never auto-flips status. CI gate fails on major drift only. |
...
- **D-5:** Hook is opt-in via `/rcode-enable-hooks`. Off by default. ✓
...
| # | Verification |
|---|---|
| #461 | `/rcode-feature-drift --mode=phase-status` invocable; CI dogfood gate Check 6 confirms phase-status alignment on every run. |
...
- Implementing `/rcode-enable-hooks` skill body changes — Phase 8 documented the opt-in path; the skill itself can be authored when the rcode-skills team has bandwidth (separate ticket).
