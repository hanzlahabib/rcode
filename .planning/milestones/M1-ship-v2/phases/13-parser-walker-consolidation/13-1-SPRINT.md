...
requirements_addressed:
  ...
  - "BRIEF acceptance: grep -c 'function.*[Pp]arse.*[Rr]oadmap' rcode/bin/rcode-tools.cjs rcode/bin/lib/roadmap.cjs → exactly 1"
  - "BRIEF acceptance: 132/132 existing tests still pass"
must_haves:
  truths:
    - "`node rcode/bin/rcode-tools.cjs roadmap list-phases` returns the same phases as before this sprint"
    - "`node rcode/bin/rcode-tools.cjs roadmap get-phase 6` returns found:true with name+goal+requirements+plans"
    - "`node rcode/bin/rcode-tools.cjs progress init` returns the same phase set in its output as before"
    - "`node rcode/bin/rcode-tools.cjs state sync --from-disk` reports phases_found > 0 (no #455 regression)"
    - "All 132/132 existing tests still pass"
  artifacts:
    - "rcode/bin/lib/roadmap.cjs exports extractPhases (already does) — only one parser implementation in the entire codebase"
    - "rcode/bin/rcode-tools.cjs no longer defines parseRoadmapPhases nested in cmdProgress"
    - "rcode/bin/rcode-tools.cjs no longer contains the inline /^#{2,4}\\s*Phase\\s+(\\d{1,3})\\b/gm regex inside cmdState's --from-disk branch"
  key_links:
    - "cmdProgress (rcode-tools.cjs:3879) calls extractPhases via require('./lib/roadmap.cjs')"
    - "cmdState --from-disk branch (rcode-tools.cjs:~2467) calls extractPhases via require('./lib/roadmap.cjs')"
...
<objective>
Collapse three ROADMAP parsers down to one — `extractPhases` in `rcode/bin/lib/roadmap.cjs`. Delete `parseRoadmapPhases` (nested in `cmdProgress` at line 3894) and the inline `/^#{2,4}\s*Phase\s+(\d{1,3})\b/gm` regex inside cmdState's `--from-disk` branch (line ~2474). Both callers must use the canonical helper.

Purpose: Drift between three implementations caused #460/#462/#464/#465. One implementation cannot drift from itself.

Output: `rcode/bin/lib/roadmap.cjs` is the single source of truth for ROADMAP phase parsing. Verified by grep + 132/132 tests.
</objective>
...
<files>
- rcode/bin/lib/roadmap.cjs (lines 21-65, the extractPhases function)
</files>

<read_first>
- rcode/bin/lib/roadmap.cjs (full file — 280 lines)
- rcode/bin/rcode-tools.cjs lines 3894-3930 (parseRoadmapPhases — note its dual-format Format A pipe / Format B heading logic and its `seen` deduplication)
</read_first>

<action>
The current `extractPhases` regex at line 26 only matches heading-style (`## Phase N — Name`). The about-to-be-deleted `parseRoadmapPhases` (rcode-tools.cjs:3894-3930) also handled pipe-table format (`| 07 | Name | Goal |`) with deduplication via a `seen` Set. To replace both call sites without behavior loss, extractPhases must support both formats.
...
Why: The current ROADMAP.md uses heading-style exclusively (verified by `grep '^|' .planning/ROADMAP.md` = 0 hits). But `parseRoadmapPhases` supported both formats defensively for downstream consumers / other repos using rcode. Behavior preservation requires the canonical helper to do the same.
</action>

<verify>
<automated>
cd /home/hanzla/development/rcode && node -e "
const { dispatch } = require('./rcode/bin/lib/roadmap.cjs');
...
cd /home/hanzla/development/rcode && node -e "
const { extractPhases } = (() => { const m = require('./rcode/bin/lib/roadmap.cjs'); return { extractPhases: m.extractPhases || require('./rcode/bin/lib/roadmap.cjs').__esModule ? null : null }; })();
// Fall back: re-require if extractPhases isn't exported
const mod = require('./rcode/bin/lib/roadmap.cjs');
...
<files>
- rcode/bin/rcode-tools.cjs (delete function at lines 3894-3930; update its single caller inside cmdProgress)
</files>

<read_first>
- rcode/bin/rcode-tools.cjs lines 3879-4100 (the entire cmdProgress function — find the call site of parseRoadmapPhases via grep)
- rcode/bin/lib/roadmap.cjs (after Story 13.1.1 lands — confirm extractPhases is exported)
</read_first>

<action>
1. Find every call to `parseRoadmapPhases()` inside cmdProgress (rcode-tools.cjs ~lines 3879-end of cmdProgress). Use `grep -n "parseRoadmapPhases" rcode/bin/rcode-tools.cjs` — there should be exactly one definition (line 3894) and one or more calls.
...
<verify>
<automated>
cd /home/hanzla/development/rcode && grep -c "function parseRoadmapPhases" rcode/bin/rcode-tools.cjs | grep -q "^0$" && echo "PASS: parseRoadmapPhases definition deleted" || { echo "FAIL: parseRoadmapPhases still defined"; exit 1; }
</automated>

<automated>
cd /home/hanzla/development/rcode && grep -c "parseRoadmapPhases(" rcode/bin/rcode-tools.cjs | grep -q "^0$" && echo "PASS: no callers" || { echo "FAIL: orphan callers remain"; grep -n "parseRoadmapPhases" rcode/bin/rcode-tools.cjs; exit 1; }
</automated>

<automated>
cd /home/hanzla/development/rcode && node rcode/bin/rcode-tools.cjs progress init 2>&1 | head -40 | grep -q '"phases"' && echo "PASS progress init renders phases" || { echo "FAIL: progress init broken"; node rcode/bin/rcode-tools.cjs progress init 2>&1 | head -40; exit 1; }
</automated>
</verify>

<done>
- `grep "function parseRoadmapPhases" rcode/bin/rcode-tools.cjs` returns 0 matches
- `grep "parseRoadmapPhases(" rcode/bin/rcode-tools.cjs` returns 0 matches
- `node rcode/bin/rcode-tools.cjs progress init` produces phases output identical in shape to before
- `node rcode/bin/rcode-tools.cjs progress bar --raw` still produces an ASCII bar (unchanged)
</done>
...
<files>
- rcode/bin/rcode-tools.cjs (lines ~2460-2478 inside cmdState's add-phase / sync flow)
</files>

<read_first>
- rcode/bin/rcode-tools.cjs lines 2440-2510 (full context of the cmdState branch that uses `pipeRe` and `headRe` to compute `maxNum`)
</read_first>

<action>
At rcode-tools.cjs ~line 2467-2477, the code computes `maxNum` by running TWO inline regexes against ROADMAP.md:
```js
...
<verify>
<automated>
cd /home/hanzla/development/rcode && grep -nE "/\^\#\{2,4\}\\\\s\*Phase|pipeRe\s*=|headRe\s*=" rcode/bin/rcode-tools.cjs | grep -v "^\s*//" && { echo "FAIL: inline regex still present"; exit 1; } || echo "PASS: inline phase regex removed"
</automated>

<automated>
cd /home/hanzla/development/rcode && node rcode/bin/rcode-tools.cjs state sync --from-disk 2>&1 | grep -q '"phases_found": [1-9]' && echo "PASS: state sync reports phases_found >= 1 (no #455 regression)" || { echo "FAIL: state sync broken"; node rcode/bin/rcode-tools.cjs state sync --from-disk 2>&1 | head -20; exit 1; }
</automated>

<automated>
# Final consolidation gate — exactly one parser function remaining
cd /home/hanzla/development/rcode && total=$(grep -hE "function.*[Pp]arse.*[Rr]oadmap|^function extractPhases" rcode/bin/rcode-tools.cjs rcode/bin/lib/roadmap.cjs | wc -l); [ "$total" = "1" ] && echo "PASS: exactly 1 parser definition" || { echo "FAIL: $total parser definitions"; grep -nE "function.*[Pp]arse.*[Rr]oadmap|^function extractPhases" rcode/bin/rcode-tools.cjs rcode/bin/lib/roadmap.cjs; exit 1; }
</automated>

<automated>
cd /home/hanzla/development/rcode && pnpm test 2>&1 | tail -20 | grep -qE "132 (passed|passing)" && echo "PASS 132 tests" || { echo "FAIL: tests broken"; pnpm test 2>&1 | tail -40; exit 1; }
</automated>
</verify>
...
- Acceptance gate: `grep -c "function.*[Pp]arse.*[Rr]oadmap" rcode/bin/rcode-tools.cjs rcode/bin/lib/roadmap.cjs` shows exactly 1 total (the `extractPhases` definition in lib/roadmap.cjs)
...
<verification>
- `grep -c "function.*[Pp]arse.*[Rr]oadmap" rcode/bin/rcode-tools.cjs rcode/bin/lib/roadmap.cjs` → exactly 1 (only `extractPhases` in lib/roadmap.cjs counts; we add a `function extractPhases` count too via the regex hit)
- `node rcode/bin/rcode-tools.cjs roadmap list-phases` returns ≥10 phases
- `node rcode/bin/rcode-tools.cjs roadmap get-phase 6` returns `"found": true`
- `node rcode/bin/rcode-tools.cjs progress init` returns phases in expected shape
- `node rcode/bin/rcode-tools.cjs state sync --from-disk` reports `phases_found >= 1`
