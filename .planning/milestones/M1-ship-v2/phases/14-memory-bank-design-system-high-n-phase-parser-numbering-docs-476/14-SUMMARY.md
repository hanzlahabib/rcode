...
- `rcode/templates/memory/project/design-system.md` — full template with 5 sections (Tokens, Canonical components, Conventions, RTL/multilingual, Inspiration/lineage)
- `rcode/templates/memory/INDEX.md` — added directory-map row pointing to the new template, with the read-by-agents callout (`ui-phase`, `frontend-design`, `clone-website`)
- `rcode/skills/core/rcode-memory-init/SKILL.md` — output summary lists `design-system.md` as a still-empty file new projects should fill in
...
- All 9 `\d{1,3}` regex sites in `rcode-tools.cjs` replaced with `\d+`. Locations:
...
- New: `docs/phase-numbering.md` — names 4 supported options (sequential, decimal sub-phases, parking lot 999.x, hot-track 1000+) with tradeoffs and a recommendation: **decimal sub-phases (Option B)** for hot-fixes since `/rcode-insert-phase --number 100.1` already supports them and `lib/roadmap.cjs` already parses them
...
- `grep '\\\\d{1,3}' rcode/bin/rcode-tools.cjs rcode/bin/lib/*.cjs` → 0 functional matches (only comments referencing the old pattern for context)
...
- **Skill-name resolver** (`Skill(rcode-ui-phase)` rejected, must be `Skill(rcode-ui-phase)`) — Claude Code namespacing issue, not a rcode bug per se. File separately.
