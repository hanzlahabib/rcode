...
must_haves:
  ...
  artifacts:
    - .rihal/references/iterative-retrieval.md (the loop contract, @-includable)
    - .rihal/references/agent-contracts.md (loop contract documented)
  key_links:
...
`research-phase.md` Step 5 "Handle Return" (lines ~85-92) is the insertion point for the per-phase loop. `new-project-research.md` spawns 4 parallel `rihal-project-researcher` agents (Stack/Features/Architecture/Pitfalls) — the loop applies per-dimension after they return.
The 4 research subagents in scope: `rihal-phase-researcher`, `rihal-project-researcher`, `rihal-research-synthesizer`, `rihal-codebase-mapper` — all confirmed present in `rihal/agents/`.
...
<action>
Create `.rihal/references/iterative-retrieval.md` — a concise reference defining the bounded iterative-retrieval loop, written so a research-spawning workflow can `@`-include it. It MUST specify:
...
Then update `.rihal/references/agent-contracts.md`: add a short section "Iterative retrieval (research subagents)" that summarizes the loop in 2-3 sentences and points to `iterative-retrieval.md` for the full contract.
</action>
<acceptance_criteria>
- `.rihal/references/iterative-retrieval.md` exists and contains the strings `3 cycles` (or `3 cycle`), `objective`, and `executor` (the out-of-scope statement).
- `grep -qi "iterative" .rihal/references/agent-contracts.md` succeeds.
- `grep -q "iterative-retrieval.md" .rihal/references/agent-contracts.md` succeeds.
</acceptance_criteria>
<verify>
<automated>
test -f .rihal/references/iterative-retrieval.md && grep -qi "3 cycle" .rihal/references/iterative-retrieval.md && grep -qi "executor" .rihal/references/iterative-retrieval.md && grep -q "iterative-retrieval.md" .rihal/references/agent-contracts.md
</automated>
</verify>
...
<action>
Edit `rihal/workflows/research-phase.md`:
...
2. In Step 5 (Handle Return), insert a new sub-step before the existing bullets: `@.rihal/references/iterative-retrieval.md`, then a prose block instructing the orchestrator to evaluate the returned `## RESEARCH COMPLETE` summary for sufficiency against the Step-4 objective and, if insufficient (or `## RESEARCH INCONCLUSIVE`), re-dispatch `rihal-phase-researcher` with a follow-up prompt naming the gaps — capped at 3 total cycles. After the cap, proceed with the best result and note residual gaps.
Keep all existing Step 5 routing bullets (`Plan/Dig deeper/Review/Done`) intact — the loop runs before they are offered.
</action>
<acceptance_criteria>
- `grep -q "iterative-retrieval.md" rihal/workflows/research-phase.md` succeeds.
- `grep -qi "3 cycle\|cap" rihal/workflows/research-phase.md` succeeds.
- The `<objective>` block in Step 4 references the broader phase goal: `grep -qi "objective_context\|why this phase\|broader" rihal/workflows/research-phase.md` succeeds.
</acceptance_criteria>
<verify>
<automated>
grep -q "iterative-retrieval.md" rihal/workflows/research-phase.md && grep -qi "cycle" rihal/workflows/research-phase.md && grep -qi "objective_context\|broader\|why this phase" rihal/workflows/research-phase.md
</automated>
</verify>
...
<action>
Edit `rihal/workflows/new-project-research.md`:
...
2. After the 4 parallel researchers return (locate the point where their results are collected — after the 4 `Task(...)` calls and before the synthesis/requirements step), add a new section: `@.rihal/references/iterative-retrieval.md`, then prose instructing the orchestrator to evaluate each dimension's returned artifact for sufficiency against its objective and re-dispatch that dimension's `rihal-project-researcher` with follow-up questions if insufficient — capped at 3 cycles per dimension. After the cap, proceed with the best result.
Do not alter the existing parallel-spawn structure or the synthesis step — additions only.
</action>
<acceptance_criteria>
- `grep -q "iterative-retrieval.md" rihal/workflows/new-project-research.md` succeeds.
- `grep -c "<objective>" rihal/workflows/new-project-research.md` returns at least 4 (one per dimension prompt).
- `grep -qi "cycle\|cap" rihal/workflows/new-project-research.md` succeeds.
</acceptance_criteria>
<verify>
<automated>
grep -q "iterative-retrieval.md" rihal/workflows/new-project-research.md && test "$(grep -c '<objective>' rihal/workflows/new-project-research.md)" -ge 4 && grep -qi "cycle" rihal/workflows/new-project-research.md
</automated>
</verify>
...
- `.rihal/references/iterative-retrieval.md` exists and mentions the 3-cycle cap and the executor out-of-scope statement.
- Both `rihal/workflows/research-phase.md` and `rihal/workflows/new-project-research.md` `@`-include `iterative-retrieval.md` (the `@.rihal/references/iterative-retrieval.md` reference resolves to an existing file).
...
<verification>
- `.rihal/references/iterative-retrieval.md` defines initial-objective passing, sufficiency evaluation, same-subagent re-dispatch, and the 3-cycle cap.
