...
must_haves:
  truths:
    ...
    - "BRAND.md naming reflects the actual /rcode-* command convention."
  artifacts:
  ...
  key_links:
    ...
    - "BRAND.md naming table <-> actual command files in rcode/commands/ (all named rcode-*, slash form /rcode-*)."
---

<objective>
Onboarding clarity (#758) + polish bundle (#759). Two install models exist — `npx @hanzlaa/rcode install` for project files and `npm install -g @hanzlaa/rcode` for the `rcode` PATH command — and the second surfaced only as an afterthought (README lines 103-106). `/rcode-init` vs `/rcode-new-project` is ambiguous. Differentiation is implicit (USP.md exists but README never names a competitor). examples/ has only 2 thin files. BRAND.md mandates `/rcode:` naming (line 42) while every shipped command is `/rcode-*`.
...
<type>auto</type>
<read_first>
- README.md (after sprints 30-2/30-3 — grep for "npm install -g" and "/rcode-init"; lines ~80-115 in the original)
- /home/hanzla/development/rcode/docs/getting-started.md (lines 7-30 — Install + Step 1)
- /home/hanzla/development/rcode/examples/starter-walkthrough.md (full — to see which first command it uses)
</read_first>
...
2. Pick `/rcode-init` as THE canonical first-run command (getting-started.md already leads with it; README line 111-112 already uses it). Make README and docs/getting-started.md both state `/rcode-init` as the single first command, and explicitly note that `/rcode-new-project` is a sub-path `/rcode-init` routes to for greenfield projects — so users are not left choosing.
3. Verify examples/starter-walkthrough.md opens with `/rcode-init` (or update it to).
</action>
<acceptance_criteria>
- README contains both `npx @hanzlaa/rcode install` and `npm install -g @hanzlaa/rcode` in one install section.
- README and docs/getting-started.md both name `/rcode-init` as the first command.
- examples/starter-walkthrough.md references `/rcode-init` as the entry point.
</acceptance_criteria>
<verify>
<automated>grep -q "npm install -g @hanzlaa/rcode" README.md && grep -q "/rcode-init" README.md && grep -q "/rcode-init" docs/getting-started.md && grep -q "/rcode-init" examples/starter-walkthrough.md && echo "install model + canonical command consistent"</automated>
</verify>
<done>Install model explained once up front; `/rcode-init` is the single canonical first command everywhere.</done>
<evidence>lines: README.md:103-106 (global install as trailing blockquote), README.md:111-112 (`/rcode-init`); docs/getting-started.md:21-30 (Step 1 `/rcode-init`); examples/starter-walkthrough.md exists (1.2K). grep `new-project` getting-started.md vs examples shows the ambiguity issue #758 cites.</evidence>

### Task 30.4.2 — auto — Add a differentiation table and maturity note to README
<type>auto</type>
<read_first>
- /home/hanzla/development/rcode/docs/USP.md (lines 14-16, 224-234 — existing comparison tables naming Cursor/Windsurf/CrewAI/AutoGen)
- README.md (after 30-2's diet — the "What makes rcode different" section)
- /home/hanzla/development/rcode/CHANGELOG.md (top — current version for the maturity statement)
</read_first>
...
<evidence>lines: docs/USP.md:14-16 + 224-234 (existing comparison tables naming Cursor, Windsurf, CrewAI, AutoGen); README.md:245-321 ("What makes rcode different" — currently no table, no competitor named). package.json:3 supplies the version.</evidence>

### Task 30.4.3 — auto — Flesh out examples/ with a real worked example
<type>auto</type>
<read_first>
- /home/hanzla/development/rcode/examples/starter-walkthrough.md (full — 1.2K, current thin example)
- /home/hanzla/development/rcode/examples/council-decision.md (full — 2.0K)
...
<action>
Create `examples/rental-app-walkthrough.md` — a real, end-to-end worked example that takes a concrete project (a rental-listing app) through the Golden Path: `/rcode-init` -> `/rcode-council` decision -> `/rcode-plan` -> `/rcode-execute` -> `/rcode-status`. For each step show the exact command typed and a realistic excerpt of the artifact it produces (a council-session snippet, a PLAN.md task block, a SUMMARY.md excerpt). Include the checked-in artifact excerpts inline as fenced code blocks so the example is self-contained — no external files needed. Keep it grounded in commands that actually exist (verified: `rcode/commands/init.md`, `council.md`, `plan.md`, `execute.md`, `status.md` all exist).
</action>
<acceptance_criteria>
- `examples/rental-app-walkthrough.md` exists and is non-trivial (> 60 lines).
- It references `/rcode-init`, `/rcode-council`, `/rcode-plan`, `/rcode-execute`, and `/rcode-status`.
</acceptance_criteria>
<verify>
<automated>test -f examples/rental-app-walkthrough.md && L=$(wc -l < examples/rental-app-walkthrough.md) && test "$L" -ge 60 && for c in init council plan execute status; do grep -q "/rcode-$c" examples/rental-app-walkthrough.md || { echo "missing /rcode-$c"; exit 1; }; done && echo "worked example complete ($L lines)"</automated>
</verify>
<done>examples/ has a substantial end-to-end worked example with inline artifact excerpts.</done>
<evidence>creates: examples/rental-app-walkthrough.md — examples/ today holds only council-decision.md (2.0K) and starter-walkthrough.md (1.2K), both thin per issue #759. Commands referenced verified present: `ls rcode/commands/` shows init.md, council.md, plan.md, execute.md, status.md.</evidence>

### Task 30.4.4 — auto — Reconcile BRAND.md naming with the actual /rcode-* convention
<type>auto</type>
<read_first>
- /home/hanzla/development/rcode/BRAND.md (lines 39-48 — Naming conventions table)
- A sample of `rcode/commands/` filenames (e.g. `init.md`, `council.md`, `plan.md`) — all install as `/rcode-*` slash commands
</read_first>
...
- BRAND.md slash-command examples use `/rcode-` prefix.
</acceptance_criteria>
<verify>
...
<type>auto</type>
<read_first>
- /home/hanzla/development/rcode/cli/install.js (full — find where the installer prints its final success/next-steps output)
- /home/hanzla/development/rcode/cli/postinstall.js (full — the postinstall Golden Path printer, in case the hint belongs here instead)
</read_first>
...
<verification>
- README + docs/getting-started.md + examples/ all use `/rcode-init` as the first command.
