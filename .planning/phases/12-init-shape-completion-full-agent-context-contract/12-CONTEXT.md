...
- **D-3:** Model fields (`researcher_model`, `planner_model`, `checker_model`) are resolved via the existing `cmdResolveModel` profile logic. When the agent id isn't installed (rare in v2 — `rihal-researcher`/`rihal-planner`/`rihal-checker` ship by default), the field is `null` instead of throwing.
...
- `rihal/workflows/plan.md` — line 79 contract for init sprint-plan
- `rihal/workflows/discuss-phase.md` — line 146 contract for init phase-op
- `rihal/bin/rihal-tools.cjs` — host file (cmdInit at line 274)
- `rihal/bin/lib/config.cjs` — nested YAML reader (parseNestedYaml)
- `rihal/bin/lib/roadmap.cjs` — get-phase parser (parseRequirements at line 67)
