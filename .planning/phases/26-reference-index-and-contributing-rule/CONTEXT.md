...
After phases 22-23 added 18 new reference files, `rihal/references/` has grown to 64 files.
Phase 26 adds:
1. `rihal/references/REFERENCES_INDEX.md` — catalogue of which references are loaded by which agents
2. A rule in `CONTRIBUTING.md` § "Adding a New Agent" — if agent body >100 lines, extract to references + @-include

## Current State

- `rihal/references/` has 64 .md files (up from ~46 before phases 22-23)
...
## Deliverable 1: rihal/references/REFERENCES_INDEX.md
...
Source: `rihal/references/` (tracked). Runtime: `.rihal/references/` (gitignored, installed by `cli/install.js`).
...
| auditor-shared-checklists.md | rihal-ui-auditor, rihal-security-auditor, rihal-security-adversary, rihal-edge-case-hunter, rihal-nyquist-auditor, rihal-docs-auditor |
| researcher-shared.md | rihal-phase-researcher, rihal-project-researcher, rihal-advisor-researcher, rihal-profiler |
...
1. `rihal/references/REFERENCES_INDEX.md` exists and lists all 18 cluster references with their loading agents
2. `CONTRIBUTING.md` contains the "Agent File Size Rule" subsection under "Adding a New Agent"
3. The rule text includes: ">100 lines", "extract to `rihal/references/`", "@-include" 
4. Accepted exceptions (nyquist-auditor, docs-auditor) are documented in the rule
5. `node --test` still passes (no regression)

## Files to Create / Edit

1. **NEW** `rihal/references/REFERENCES_INDEX.md`
