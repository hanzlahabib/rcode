...
@.planning/STATE.md
@.planning/ROADMAP.md
@.planning/phases/33-dashboard-command-runner-run-init-and-rihal-commands-through-the-ui/33-2-SUMMARY.md
</context>
...
1. ERROR TOAST — In the browser console, run:
   ...
   fetch('http://localhost:7718/api/run', {
     ...
     body: JSON.stringify({ storyId: 'cmd-rihal-badcmd', cmd: '/rihal-badcmd' }),
   ...
   temporarily selecting a cmd that would be rejected — or observe that /rihal-init
   does NOT produce an error toast).

2. DISABLED STATE — Click Run for /rihal-init. Confirm the Run button shows
...
3. DUPLICATE RUN PREVENTION — While /rihal-init is running (button shows "Running…"),
...
<output>
Create `.planning/phases/33-dashboard-command-runner-run-init-and-rihal-commands-through-the-ui/33-3-SUMMARY.md`
</output>
