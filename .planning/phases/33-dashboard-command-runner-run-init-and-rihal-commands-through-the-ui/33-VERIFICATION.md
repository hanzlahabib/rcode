...
A command-runner in the dashboard UI to launch `init` and other safe rihal commands via the orchestrator service (server/orchestrator.js, :7718), output streaming to the existing WebSocket terminal. A server-side allowlist (COMMAND_ALLOWLIST) is the security boundary. dashboard.js stays pure-stdlib view-only.
...
|---|-------|--------|----------|
| 1 | COMMAND_ALLOWLIST (12 commands) exists in server/orchestrator.js as a named const with security-boundary comment | VERIFIED | orchestrator.js:54-73, Set with 12 `/rihal-*` entries |
...
| 3 | Non-`cmd-` storyIds are NOT gated (existing Run buttons unaffected) | VERIFIED | behavioral test D: `phase-1` + `/rihal-execute` → HTTP 409 (running), not 403 |
...
Tests run against the live orchestrator (127.0.0.1:7718, token from ~/.rihal/orch-token):
...
| (c) cmd-* + allowlisted cmd | `{"storyId":"cmd-rihal-status","cmd":"/rihal-status"}` | NOT 403 | **HTTP 200** `{"storyId":"cmd-rihal-status","pid":...,"status":"running"}` | YES |
| (d) non-cmd storyId | `{"storyId":"phase-1","cmd":"/rihal-execute"}` | NOT 403 | **HTTP 409** (already running — no allowlist gate) | YES |

**H1 bypass fix confirmed:** empty `body.cmd` on a `cmd-` storyId now returns 403 (previously fell through via JS truthiness to `/rihal-dev-story cmd-*` fallback). Fix is at orchestrator.js:201-207.
...
1. Command picker dropdown renders visibly with 12 entries, first entry `/rihal-init`.
