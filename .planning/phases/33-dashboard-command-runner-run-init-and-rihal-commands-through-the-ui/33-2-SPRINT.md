...
must_haves:
  ...
  key_links:
    ...
    - "The storyId for a command session uses the cmd slug: 'cmd-rihal-init' for '/rihal-init'. Must satisfy STORY_ID_RE: /^[A-Za-z0-9._-]+$/ (orchestrator.js:52)."
---

<objective>
Build the command-picker UI in OrchestrationView and wire it to the existing
runAndOpenTerm/XtermPanel stack so the user can select a rihal command from a
...
feature: pick /rihal-init (or any other allowlisted command), hit Run, see output.
...
@.planning/STATE.md
@.planning/ROADMAP.md
@.planning/phases/33-dashboard-command-runner-run-init-and-rihal-commands-through-the-ui/33-1-SUMMARY.md
</context>
...
<verify>
<automated>
cd /home/hanzla/development/rihal-code && node --input-type=module << 'EOF'
...
const initEntry = ALLOWED_COMMANDS.find(c => c.cmd === '/rihal-init');
if (!initEntry) throw new Error('/rihal-init not in ALLOWED_COMMANDS');
...
lines: server/orchestrator.js:52 — STORY_ID_RE `/^[A-Za-z0-9._-]+$/` — "cmd-rihal-init"
...
2. DROPDOWN — The dropdown lists at least 10 entries. The first entry is
   "/rihal-init — initialise project workspace" (or similar label). No JavaScript
   console errors on page load.

3. LAUNCH — Select "/rihal-init" from the dropdown. Click "Run".
   Expected:
   a. The XtermPanel terminal overlay opens immediately (title: "/rihal-init (command runner)").
   b. The terminal shows output from the Claude PTY session.
   c. A new session card appears in the Orchestration grid with storyId "cmd-rihal-init".
...
7. STOP — In the Orchestration grid, the cmd-rihal-init card shows a "Stop" button
...
<success_criteria>
The Orchestration tab shows a command picker. Selecting /rihal-init and clicking Run
opens XtermPanel with live PTY output. The session appears in the grid with storyId
"cmd-rihal-init". Existing Run buttons on phase/sprint cards are unaffected.
</success_criteria>

<output>
Create `.planning/phases/33-dashboard-command-runner-run-init-and-rihal-commands-through-the-ui/33-2-SUMMARY.md`
</output>
