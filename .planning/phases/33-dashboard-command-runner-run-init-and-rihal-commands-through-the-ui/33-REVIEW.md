...
- `COMMAND_ALLOWLIST.has()` is an exact-string match. Semicolons, extra args, newlines, and case variations are all blocked (`/rihal-init; rm -rf /`, `/RIHAL-INIT`, `/rihal-init --arg` all fail the `.has()` test, confirmed by test).
...
This makes the gate fire whenever `storyId` starts with `cmd-`, regardless of whether `body.cmd` is present, empty, or falsy. A `cmd-` session with no command is rejected (403) rather than silently falling back to `/rihal-dev-story cmd-*`.
