...
**Phase:** 33 — Dashboard command runner (run /rcode-init and rcode commands through the UI)
...
- Task 33.1.3 is a `checkpoint:human-verify` — awaiting human to verify in the browser:
  ...
  4. Allowlisted cmd with `storyId: 'cmd-rcode-init'` returns HTTP 200 or 503 (not 403).
...
- [x] `rg '/rcode-init' server/orchestrator.js` → 1 hit (inside Set literal)
- [x] `node --check server/orchestrator.js` → syntax OK
- [x] dashboard.js untouched — line count and content unchanged
- [x] curl: `cmd-smoke` + `/rcode-rce-attempt` → HTTP 403 with correct JSON
- [x] curl: `cmd-rcode-init` + `/rcode-init` → HTTP 200/409 (not 403)
- [x] curl: `phase-33` + `/rcode-execute` → HTTP 200/409 (not 403) — regression clear
