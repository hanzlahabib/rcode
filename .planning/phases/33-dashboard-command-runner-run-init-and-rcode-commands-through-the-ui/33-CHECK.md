...
- `server/orchestrator.js:52` — `STORY_ID_RE = /^[A-Za-z0-9._-]+$/`. "cmd-rcode-init" satisfies this pattern. Confirmed via `node -e`.
...
- Existing Run button cmd values: Phase cards → `/rcode-execute`; Sprint cards → `/rcode-execute-sprint <id>`; Story cards → `/rcode-dev-story <id>`. All confirmed via shared.js:190,237,295.
