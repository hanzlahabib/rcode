...
must_haves:
  truths:
    ...
    - "POST /api/run with an allowlisted cmd (e.g. /rcode-init) succeeds (200) and spawns a PTY session as before — no regression to existing story/phase/sprint runs."
...
orchestrator falls back to the default `/rcode-dev-story ${storyId}` composition
...
- Do NOT gate the `/rcode-dev-story` path (sprint/phase/story runs).
...
const hasInit = src.includes('/rcode-init');
const hasStatus = src.includes('/rcode-status');
...
if (!hasInit) throw new Error('/rcode-init not in allowlist');
if (!hasStatus) throw new Error('/rcode-status not in allowlist');
...
2. curl POST /api/run with body `{"storyId":"smoke-test","cmd":"/rcode-init"}` and
   Authorization: Bearer <token> → expect HTTP 200 (or 503 if pty is unavailable)
   — either is acceptable, 403 is a FAIL.
3. curl POST /api/run with body `{"storyId":"smoke-test","cmd":"/rcode-rce-attempt"}`
...
<verify>
<automated>
cd /home/hanzla/development/rcode && node server/orchestrator.js 2>&1 &
ORCH_PID=$!
sleep 1
TOK=$(node -e "
...
RESP403=$(curl -s -o /dev/null -w "%{http_code}" -X POST http://localhost:7718/api/run \
  ...
  -d '{"storyId":"smoke-test","cmd":"/rcode-rce-attempt"}')
kill $ORCH_PID 2>/dev/null
[ "$RESP403" = "403" ] && echo "OK — /rcode-rce-attempt correctly rejected with 403" || echo "FAIL — expected 403 got $RESP403"
</automated>
</verify>
<done>
- `curl POST /api/run { storyId: "cmd-smoke", cmd: "/rcode-rce-attempt" }` returns HTTP 403.
- `curl POST /api/run { storyId: "phase-33", cmd: "/rcode-execute" }` returns HTTP 200 or 503 (NOT 403 — no regression).
- `curl POST /api/run { storyId: "cmd-smoke", cmd: "/rcode-init" }` returns HTTP 200 or 503 (not 403).
</done>
<evidence>
...
4. Run the same fetch with `storyId: 'cmd-rcode-init', cmd: '/rcode-init'`:
...
- `rg '/rcode-init' server/orchestrator.js` → 1 hit (inside the Set literal)
...
<output>
Create `.planning/phases/33-dashboard-command-runner-run-init-and-rcode-commands-through-the-ui/33-1-SUMMARY.md`
</output>
