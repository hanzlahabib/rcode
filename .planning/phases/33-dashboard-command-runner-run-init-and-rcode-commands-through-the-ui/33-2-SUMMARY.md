...
- **33.2.4 (human-verify) — REQUIRES IN-BROWSER VERIFICATION** at `http://localhost:7717`.
  ...
  2. Dropdown lists >= 10 entries; first entry is `/rcode-init`.
  3. Selecting `/rcode-init` and clicking Run opens XtermPanel with live PTY output.
  4. A session card appears in the grid with storyId `cmd-rcode-init`.
