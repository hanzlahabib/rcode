# Project Brief

**Project:** rihal-code
**Created:** 2026-05-16

## Overview

Rihal Code (`rcode`) is an AI team methodology packaged as an npm tool, published as
`@hanzlaa/rcode`. A single install gives an AI IDE a persistent project brain — ~45
specialist agents, 109 commands, and a file-based Memory Bank that survives session resets.
It targets developers using Claude Code, Cursor, Gemini, VS Code, and Antigravity who lose
context every time their AI assistant resets. Rihal fixes this with checked-in state and
opinionated research → plan → execute → verify → recover workflows, plus a Diwan dashboard
for viewing project state, decisions, and memory in one place.

## Technical Stack

- **Language:** JavaScript (Node.js, ESM + .cjs tooling)
- **Framework:** CLI tool; dependency-free dashboard server (Node stdlib); Preact dashboard client
- **Package manager:** pnpm / npm

## Project Goals

_To be refined. Run `/rihal-create-prd` for full requirements discovery._
