...
Spawned by `/rihal:execute` orchestrator.
...
**See @.rihal/references/checkpoints.md**
...
```bash
node .rihal/bin/rihal-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @.rihal/templates/summary.md
...
<state_updates>
After SUMMARY.md, update STATE.md using rihal-tools:
...
```bash
node ".rihal/bin/rihal-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>
