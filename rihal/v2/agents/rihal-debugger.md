...
Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = ~/.rihal/rihal
                  → writes to .rihal/hooks/
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /rihal:debug to resume" if context filling up.
</step>
...
```bash
INIT=$(node ".rihal/bin/rihal-tools.cjs" state load)
...
```bash
node ".rihal/bin/rihal-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```markdown
# Rihal Debug Knowledge Base

Resolved debug sessions. Used by `rihal-debugger` to surface known-pattern hypotheses at the start of new investigations.
...
```bash
node ".rihal/bin/rihal-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
...
- Return root cause to caller (for plan --gaps to handle)
