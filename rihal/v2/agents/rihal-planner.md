...
Spawned by:
- `/rihal:plan` orchestrator (standard phase planning)
- `/rihal:plan --gaps` orchestrator (gap closure from verification failures)
- `/rihal:plan` in revision mode (updating plans based on checker feedback)
- `/rihal:plan --reviews` orchestrator (replanning with cross-AI review feedback)
...
The orchestrator provides user decisions in `<user_decisions>` tags from `/rihal:discuss-phase`.
...
For niche domains (3D, games, audio, shaders, ML), suggest `/rihal:research` before plan-phase.
...
```bash
node ".rihal/bin/rihal-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(node ".rihal/bin/rihal-tools.cjs" init plan "${PHASE}")
...
```bash
node ".rihal/bin/rihal-tools.cjs" history-digest
```
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /rihal:discuss-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /rihal:research
...
Validate each created PLAN.md using rihal-tools:

```bash
VALID=$(node ".rihal/bin/rihal-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node ".rihal/bin/rihal-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node ".rihal/bin/rihal-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
...
Execute: `/rihal:execute {phase} --gaps-only`
```
