...
platforms with no shared methodology layer. Rihal is uniquely positioned to be the
...
building on the open-source skills CLI ecosystem, Rihal offloads the highest-churn
...
```
## Differentiation
- Anti-fragmentation positioning: Rihal = cross-platform constant across 40+ fragmenting AI tools; no competitor provides shared methodology layer
- Platform complexity delegated to Vercel skills CLI ecosystem (MIT); Rihal maintains methodology, not platform configs
```
...
- **Key gap**: NO competitor combines structured methodology with plugin
  marketplace — this is Rihal's whitespace
```
...
```
- rihal-init: always included as base skill in every bundle (solves bootstrapping)
```
...
type: rihal-distillate
sources:
  - "product-brief-rihal-next-gen-installer.md"
  - "product-brief-rihal-next-gen-installer-discovery-notes.md"
downstream_consumer: "PRD creation"
created: "2026-03-13"
token_estimate: 1450
parts: 1
...
- Plugins: skill bundles with Anthropic plugin standard as base format + rihal-manifest.json extending for Rihal-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
- Existing manifest example: `{"module-code":"rihal","replaces-skill":"rihal-create-product-brief","capabilities":[{"name":"create-brief","menu-code":"CB","supports-headless":true,"phase-name":"1-analysis","after":["brainstorming"],"before":["create-prd"],"is-required":true}]}`
- Vercel skills CLI handles platform translation; integration pattern (wrap/fork/call) is PRD decision
- rihal-init: global skill scanning installed rihal-manifest.json files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
- rihal-update: plugin update path without full reinstall; technical approach (diff/replace/preserve customizations) is PRD decision
...
## Differentiation
- Anti-fragmentation: Rihal = cross-platform constant; no competitor provides shared methodology layer across AI tools
- Curated quality: all submissions gated, human-reviewed by Rihal + core team; 13.4% of community skills have critical vulnerabilities (Snyk 2026); quality gate value increases as ecosystem gets noisier
- Domain-agnostic: no competitor builds beyond software dev workflows; same plugin system powers any domain via Rihal Builder (separate initiative)
...
- Future plugin creators: non-dev authors using Rihal Builder; need distribution without building own installer
...
- rihal-init discovers/registers all plugins from manifests; clear errors for malformed manifests
- At least one external module author successfully publishes plugin using manifest system
- rihal-update works without full reinstall
- Existing CLI users have documented migration path

## Scope
- In: manifest spec, rihal-init, rihal-update, Vercel CLI integration, NPX installer, zip bundles, migration path
- Out: Rihal Builder, marketplace web platform, skill conversion (prerequisite, separate), one-click install for all platforms, monetization, quality certification process (gated-submission principle is architectural requirement; process defined separately)
...
- Entry: `tools/cli/rihal-cli.js` (Commander.js) → `tools/cli/installers/lib/core/installer.js`
...
- Skills already use directory-per-skill layout; rihal-manifest.json sidecars exist but are not source of truth
...
- rihal-update mechanics: diff/replace? Preserve user customizations?
...
- rihal-manifest.json as open standard submission to Agent Skills governance?
...
## Opportunities
- Module authors as acquisition channel: each published plugin distributes Rihal to creator's audience
- CI/CD integration: rihal-init as pipeline one-liner increases stickiness
- Educational institutions: structured methodology + non-technical install → university AI curriculum
- Skill composability: mixing Rihal modules with third-party skills for custom methodology stacks
