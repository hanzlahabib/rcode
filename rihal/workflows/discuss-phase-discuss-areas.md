...
**Research-before-questions mode:** Check if `workflow.research_before_questions` is enabled in config (from init context or `.rihal/config.yaml` via `node rihal-tools.cjs config-get workflow.research_before_questions`). When enabled, before presenting questions for each area:
