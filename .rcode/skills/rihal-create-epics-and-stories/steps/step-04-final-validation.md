...
After saving `.planning/epics.md`, you MUST sync state so `.rcode/state.json` reflects the new epics. Without this, downstream workflows (`/rihal-status`, `/rihal-progress`, `/rihal-execute`) see a divergent picture.
...
Epics and Stories complete. Invoke the `rihal-help` skill.
