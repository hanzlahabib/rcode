...
- 🚷 **NEVER** invent an "autonomous mode" or any self-declared bypass. The only sanctioned bypass paths are `.rcode/config.yaml` → `mode: yolo` or `/rihal-do --auto`. See `../../_shared/no-autonomous-bypass.md` for the full rule and correct response pattern.
...
1. **Search for whole document first** - Look for `epics.md`, `rihal-epics.md`, or any `*epic*.md` file
