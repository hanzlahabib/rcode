...
HIGHEST=$(node ".rcode/bin/rihal-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node ".rcode/bin/rihal-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
