...
3c. **Auth-strategy check (standing — re-evaluate here, keyword generation alone is not enough)** — Gray-area generation in step 3 can surface "Session handling" or "Error responses" for an auth-flavored phase while never asking which production auth strategy is being built against — the exact fragility the entry-point check above exists to guard against, applied to auth. At this point in the analysis, ask instead: **did step 3 surface any auth/login/session-category gray area, or does the phase goal contain auth/login/session/SSO/account signals?** If yes, always ask what the production authentication strategy is (SSO/Entra/OAuth, local accounts, magic link, etc.) and whether any temporary/dev-only bypass is explicitly scoped out of this phase and tracked as its own follow-up. This must not be silently skipped for an auth/login/session phase — either surface it as a selectable gray area in `present_gray_areas` (e.g., "Auth strategy — SSO/Entra/OAuth, local accounts, or magic link? Any dev-only bypass tracked separately?") or, if the user defers it, record it explicitly under `## Claude's Discretion` in CONTEXT.md. Never omit it entirely; it is how a dev-only bypass cookie ships while the real strategy discussion never happens.

3d. **Roles/Permissions check (standing — for any phase or project with >1 user role)** — If the phase or project involves more than one user role (check REQUIREMENTS.md, PROJECT.md, and the phase description for role/permission language), do not assume a single shared UI is safe by default. Ask: what does each role see differently in nav/menus/dashboards, which screens exist for one role but not another, which fields/actions are hidden vs. merely disabled per role, and what does the no-permission state look like? See `domain-probes.md`'s Roles & Permissions section for the full question bank. This must not be silently skipped just because the ROADMAP heading doesn't literally say "UI" — either surface it as a selectable gray area in `present_gray_areas` or, if deferred, record it explicitly under `## Claude's Discretion` in CONTEXT.md. This is the standing check for whether a multi-role phase needs distinct per-role screens/nav, not just whether one feature is discoverable (that's 3b).
