...
3. All `*-SPRINT.md` files in the phase directory
...
Write to a temp file: `/tmp/rcode-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/rcode-review-prompt-{phase}.md)" 2>/dev/null > /tmp/rcode-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/rcode-review-prompt-{phase}.md)" 2>/dev/null > /tmp/rcode-review-claude-{phase}.md
```

**Codex:**
```bash
codex exec --skip-git-repo-check "$(cat /tmp/rcode-review-prompt-{phase}.md)" 2>/dev/null > /tmp/rcode-review-codex-{phase}.md
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/rcode-review-coderabbit-{phase}.md
```

**OpenCode (via GitHub Copilot):**
```bash
cat /tmp/rcode-review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/rcode-review-opencode-{phase}.md
if [ ! -s /tmp/rcode-review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/rcode-review-opencode-{phase}.md
fi
```
...
plans_reviewed: [{list of SPRINT.md files}]
---
...
Commit:
```bash
node ".rcode/bin/rcode-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/rcode-plan --reviews)
</success_criteria>
