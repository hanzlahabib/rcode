...
// keeps thin zero-arg/closure wrappers so the ~100 existing call sites in
// this file (readState(), writeState(state), etc.) don't all need to start
// threading STATE_PATH/RCODE_DIR/PROJECT_ROOT through by hand.
const PHASE_STATUS_ALIASES = stateIO.PHASE_STATUS_ALIASES;
const PHASE_STATUS_ENUM = stateIO.PHASE_STATUS_ENUM;
const normalizePhaseStatus = stateIO.normalizePhaseStatus;
const migrateState = stateIO.migrateState;

/** Read state or return default skeleton. */
function readState() {
  return stateIO.readState(STATE_PATH, RCODE_DIR, PROJECT_ROOT);
}

/** Atomic write: write to temp file then rename. */
function writeState(state) {
  return stateIO.writeState(state, STATE_PATH, RCODE_DIR, PROJECT_ROOT);
}

/** Write state and return compact result (hides full state from output) */
function writeStateCompact(state, meta) {
  return stateIO.writeStateCompact(state, meta, STATE_PATH, RCODE_DIR, PROJECT_ROOT);
}

function defaultState(projectName) {
  return stateIO.defaultState(projectName, PROJECT_ROOT);
}
...
function cmdState(subArgs) {
  ...
  // parseFlags/globalDecisionsPath/appendGlobalDecision/readGlobalDecisions
  // moved to lib/state-io.cjs (#204 step 2). Thin wrappers below preserve
  // every existing call site in this function (parseFlags(1), etc.).
  const parseFlags = (startIdx) => stateIO.parseFlags(subArgs, startIdx);
  const globalDecisionsPath = stateIO.globalDecisionsPath;
  const appendGlobalDecision = stateIO.appendGlobalDecision;
  const readGlobalDecisions = stateIO.readGlobalDecisions;

  // Shared deps for the state-*.cjs family modules (#204 step 3). subArgs
  // is threaded through unchanged, so the already-subArgs-bound parseFlags
  // above works as a drop-in for every family's parseFlags(N) call site.
  const stateDeps = {
    readState, writeState, writeStateCompact, defaultState, migrateState,
    parseFlags, globalDecisionsPath, appendGlobalDecision, readGlobalDecisions,
  ...
  // --- read / get ---
  {
    ...
    const r = lifecycleLib.dispatch(subArgs, stateDeps);
    if (r !== undefined) return r;
  }
  {
    const phaseLifecycleLib = require(path.join(__dirname, 'lib', 'state-phase-lifecycle.cjs'));
    const r = phaseLifecycleLib.dispatch(subArgs, stateDeps);
    if (r !== undefined) return r;
  }
  {
    const sprintLib = require(path.join(__dirname, 'lib', 'state-sprint.cjs'));
    const r = sprintLib.dispatch(subArgs, stateDeps);
    if (r !== undefined) return r;
  }
  {
    const decisionsLib = require(path.join(__dirname, 'lib', 'state-decisions.cjs'));
    const r = decisionsLib.dispatch(subArgs, stateDeps);
    if (r !== undefined) return r;
  }
  {
    const workstreamsLib = require(path.join(__dirname, 'lib', 'state-workstreams.cjs'));
    const r = workstreamsLib.dispatch(subArgs, stateDeps);
    if (r !== undefined) return r;
  }
  {
    const idsLib = require(path.join(__dirname, 'lib', 'state-ids.cjs'));
    const r = idsLib.dispatch(subArgs, stateDeps);
    if (r !== undefined) return r;
  }
  {
    const schemaLib = require(path.join(__dirname, 'lib', 'state-schema.cjs'));
    const r = schemaLib.dispatch(subArgs, stateDeps);
    if (r !== undefined) return r;
  }
  ...
  try {
    ...
    switch (subcommand) {
      ...
      case 'state':
        ...
        break;
      case 'phase': {
        const phaseLib = require(path.join(__dirname, 'lib', 'phase.cjs'));
        result = phaseLib.cmdPhase(args, {
