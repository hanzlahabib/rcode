...
1. **Method registry loading.** Read `./methods.csv` and `{agent_party}` from `.rcode/team.yaml`.
...
**Negative — wrong skill** — `review this code for bugs` is code review, not elicitation. Route to `rcode-review`.

## Memory Bank Hooks

- **Reads:** `methods.csv`, `.rcode/team.yaml` (agent_party), the section content being enhanced
