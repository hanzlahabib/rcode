...
1. **Load config by reading @.rcode/skills/rcode-init/SKILL.md** — Store `{user_name}`, `{communication_language}`, vars.
...
4. **Present the capabilities table** and remind the user they can invoke `rcode-help`.
...
**Expected behavior:** Ask for the story file path. If no story exists, respond: "No story found. Request Hussain (rcode-agent-hussain-pm) to run `rcode-create-story` for user login first. I execute approved stories — I don't invent scope." Do NOT start implementing.
