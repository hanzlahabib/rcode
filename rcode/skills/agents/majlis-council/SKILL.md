...
8. **Save the session** — record to `.rcode/progress/majlis-{date}.md` for audit.
...
9. **Saved to:** `.rcode/progress/majlis-{date}.md`
...
"Should we pivot our product from government clients to private enterprise?" → Council = Sadiq, Hussain-PM, Mariam, Waleed, Khalid, Noor → positions table → alignment + dissent → Khalid's dissent is load-bearing → 3 paths (full pivot / hybrid / stay course) → decision owner Sadiq → save to `.rcode/progress/majlis-2026-04-10.md`.
...
- **Reads:** `rcode/team.yaml`, `.rcode/state.json`, `.rcode/context/active.md`, every consulted specialist's SKILL.md
- **Writes:** `.rcode/progress/majlis-{date}.md` (session record)
