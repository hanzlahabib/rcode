...
If the user asks to "validate" or "polish" an existing PRD, redirect to `rcode-validate-prd`.
...
**Graceful redirect:** If after 2-3 exchanges the user can't articulate a customer or problem, don't force it — suggest the idea may need more exploration first and recommend they invoke the `rcode-brainstorming` skill to develop it further.
...
- Complete PRFAQ document at `{planning_artifacts}/prfaq-{project_name}.md` using `./assets/prfaq-template.md`
- PRD distillate block at end of document for downstream consumption
- Sections: Press Release (1 page), Customer FAQ (5-10 Q&As), Internal FAQ (risks/feasibility/trade-offs), Verdict (build/refine/kill)
- Max 3 pages total

## Examples

### Happy Path
**Input:** "Create a PRFAQ for an AI PRD validator"
...
### Headless Mode
**Input:** `/rcode-prfaq --headless --customer "{persona}" --problem "{problem}" --stakes "{why}" --solution "{concept}"`
...
Do NOT use this skill for writing a full PRD (use rcode-create-prd), validating an existing PRD (use rcode-validate-prd), market research (use rcode-sadiq-analyst), or sprint planning. PRFAQ is a concept-forging challenge, not a requirements document.
