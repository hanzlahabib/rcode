...
1. **Create a failing test first.** Simplest possible reproduction. Use `rcode-prove-it` for writing the test that locks the fix in.
...
Regression test:
  <hand to rcode-prove-it — the test that locks the fix in>
```
