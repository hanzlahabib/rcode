<purpose>
Create all phases necessary to close gaps identified by `/rcode-audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/rcode-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/rcode-audit-milestone` first.
```
...
HIGHEST=$(node ".rcode/bin/rcode-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node ".rcode/bin/rcode-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/rcode-execute {N}` — if plans already exist
...
`/rcode-audit-milestone` — re-audit to verify gaps closed
`/rcode-complete-milestone {version}` — archive when audit passes
```
