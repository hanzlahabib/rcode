...
> Skip this section entirely if workflow.nyquist_validation is explicitly set to false in .rcode/config.yaml. If the key is absent, treat as enabled.
...
- **Phase gate:** Full suite green before `/rcode-verify-work`
...
Also read `workflow.nyquist_validation` from `.rcode/config.yaml` (via `node rcode-tools.cjs config-get workflow.nyquist_validation`) — include Validation Architecture section in RESEARCH.md unless the value is explicitly `false`. If the key is absent or `true`, include the section.
...
```bash
node ".rcode/bin/rcode-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
