...
#
# When an Edit/Write touches rihal/bin/**, sync the source to .rihal/bin/ so
...
# `tool_input.file_path` and acts only if it's under rihal/bin/.
...
case "$file_path" in
  */rihal/bin/*)
    ...
    [ -n "$REPO_ROOT" ] || exit 0
    [ -d "$REPO_ROOT/.rihal/bin" ] || exit 0
    cp -r "$REPO_ROOT/rihal/bin/." "$REPO_ROOT/.rihal/bin/"
    echo "[sync-bin] rihal/bin/ → .rihal/bin/ (after edit to $file_path)" >&2
    ;;
esac
