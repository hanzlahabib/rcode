# Rihal Code — Worked Example: A Rental-Listing App
...
The path is: `/rihal-init` → `/rihal-council` → `/rihal-plan` → `/rihal-execute` → `/rihal-status`.
...
## 1. Initialize — `/rihal-init`

`/rihal-init` is the single entry point. It detects this is an empty directory, so it
routes into the greenfield sub-path automatically — you do not call `/rihal-new-project`
yourself.

```
/rihal-init rental-listing app for a Dubai property agency
```
...
```
.rihal/config.yaml          # language: en, profile: balanced
.rihal/RIHLA.md             # project identity + conventions
...
## 2. Decide the hard question — `/rihal-council`
...
```
/rihal-council should listings search be Postgres full-text or a dedicated search index?
```
...
The decision is logged to `.rihal/decisions.jsonl` and is now visible to every later
agent — the planner will not re-litigate it.

---

## 3. Plan the first phase — `/rihal-plan`

With the decision settled, plan phase 1. `--research` grounds the plan in the codebase
and the council decision first.

```
/rihal-plan --research phase 1: listing CRUD + Postgres search
```

The planner runs, then `rihal-plan-checker` validates every file/symbol reference.
...
## 4. Execute the plan — `/rihal-execute`

The executor works task by task, makes one atomic commit per task, and runs the
post-execute gates.

```
/rihal-execute .planning/phases/1-listing-crud-search/PLAN.md
```
...
Post-execute gates:
- rihal-integration-checker: PASS (cross-phase E2E)
- rihal-nyquist-auditor:     PASS (search paths covered)
```

---

## 5. Check where you are — `/rihal-status`
...
Blockers .............................. none
Next .................................. /rihal-plan phase 2: viewing requests
```
...
- `/rihal-init` is always the first command — greenfield routing is automatic.
- A `/rihal-council` decision becomes durable context the planner respects.
- `/rihal-plan` is grounded and reference-checked before any code is written.
- `/rihal-execute` commits atomically and runs verification gates.
- `/rihal-status` reads project state back out at any point.

Every command here ships in the box: `rihal/commands/init.md`, `council.md`, `plan.md`,
