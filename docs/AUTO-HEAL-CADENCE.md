...
Recommended schedules for rihal-code's auto-heal tools. Pick the cadence that matches your project's velocity and ops budget.
...
```cron
# rihal-code auto-heal cadence
# Format: m h dom mon dow command

# Weekly health check (Mon 9am)
0 9 * * 1 cd /path/to/repo && pnpm dogfood >> .rihal/logs/health.log 2>&1

# Daily phase-status drift scan (6pm)
0 18 * * * cd /path/to/repo && node rihal/bin/rihal-tools.cjs roadmap list-phases > /dev/null 2>&1 && \
  echo "$(date): phase-status drift baseline OK" >> .rihal/logs/drift.log

# Monthly memory bank --fix sweep (1st of month, 9am)
0 9 1 * * cd /path/to/repo && /usr/bin/env node rihal/bin/rihal-tools.cjs state sync --from-disk \
