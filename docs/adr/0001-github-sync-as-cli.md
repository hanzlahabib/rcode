...
**Context:** A user asked whether it made sense to keep a dedicated CLI (`rcode github-sync`) for pushing phases/epics/stories to GitHub, or whether Claude in the editor should just read the `.rcode/` artifacts and run `gh` commands directly.

---

## Context

rcode produces project artifacts under `.rcode/phases/` — phase briefs, epics, stories, sprint plans. Teams want these tracked on GitHub as milestones and issues so non-rcode tooling (PR templates, project boards, release notes) can see the work.

Two credible architectures exist for getting artifacts onto GitHub:

**Option A — CLI tool (`rcode github-sync`)**
A dedicated Node.js command that walks `.rcode/phases/`, diffs against a sync map, calls `gh issue create` / `gh api` directly, and records results atomically in `.rcode/integrations/github-map.json`.
...
**We keep the CLI as the single path for GitHub mutations.** Claude slash commands (`/rcode-push-sprint`, `/rcode-push-epic`, `/rcode-push-story`) are thin wrappers that invoke the CLI. Claude never calls `gh` directly for issue creation.
...
| **Artifact preparation** — turning requirements into story files | Claude (`/rcode-generate-sprint`, `rcode-create-epics-and-stories`) | Semantic work; CLI would be a glorified template engine |
...
### 1. Idempotency
The sync map at `.rcode/integrations/github-map.json` records SHA-256 content hashes and GitHub issue numbers. Re-running sync creates zero duplicates — only new or changed items get touched. Claude would need to re-derive this mapping every session from scratch, either by searching GitHub (slow, error-prone) or by reading the JSON file (at which point it's doing the CLI's job in markdown form).
...
The 23-label rcode taxonomy (Type / Priority / Status / Area) lives in one place (`cli/github-sync.js`). If it evolves, we ship a new package version. Claude-driven creation would spread inconsistent labels across runs as different sessions interpret the rules slightly differently.
...
It can, but the CLI already reads story file content verbatim (3000-char slice) into the issue body. The surrounding scaffolding (Meta block, Parent Epic link, acceptance criteria stub) is minimal. If a team wants a different scaffold, we plan to extract templates to `.rcode/templates/github/*.md` — editable without shipping code.

### "You still need Claude for semantic work"
Yes — that's why the slash commands exist. `/rcode-push-epic` is a Claude-driven command that reads the epic, optionally offers improvements, then delegates to the CLI for the actual posting. The plumbing is deterministic; the judgment is LLM-driven. The split is the point.
...
### Positive
- **Bulk sync is fast, cheap, and reliable.** `rcode github-sync --execute` pushes dozens of issues in seconds.
...
### Negative
- **The label taxonomy is hard-coded in JavaScript.** Changing it requires a package update. Mitigation: labels are opt-in (`--with-labels`) so most users never touch them, and we plan to extract templates to `.rcode/templates/github/` in a later release.
...
- Examples: `/rcode-push-sprint`, `/rcode-push-epic`, `/rcode-push-story`
...
- `.rcode/integrations/github-map.json` — the sync map (per-project)
