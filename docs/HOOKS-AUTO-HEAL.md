...
Layer real-time drift detection on top of the manual auto-heal tools. When you edit a doc, story, or spec, a hook fires `/rcode-feature-drift --quick` automatically. Findings show up in your conversation. The hook **never modifies files** — it's strictly report-only.
...
Runs `/rcode-feature-drift --quick` against the project, surfaces any drift the audit finds. Target runtime: **<2 seconds**. Designed not to block your edit flow.
...
This is enforced in `rcode/workflows/feature-drift.md` parse_args step:
...
## Opt-in via `/rcode-enable-hooks`

The hook is **off by default**. To enable, run `/rcode-enable-hooks` (or paste the JSON block below into your `.claude/settings.json` manually).
...
2. Spawns `/rcode-feature-drift --quick` via the same Skill mechanism Claude Code uses internally.
...
- Or run `/rcode-enable-hooks --disable` (if the skill exposes a teardown path)

Disabling is reversible — re-running `/rcode-enable-hooks` puts it back.
...
**Skip** if:
- You're a solo dev with a small project — manual `/rcode-feature-drift` weekly is enough
...
The hook complements `/rcode-feature-drift` on its scheduled cadence (see [AUTO-HEAL-CADENCE.md](AUTO-HEAL-CADENCE.md)):
